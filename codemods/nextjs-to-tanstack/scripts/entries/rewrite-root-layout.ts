...
import { readResolvedI18nConfig, type NextI18nCodemodConfig } from '../utils/read-next-i18n-config.ts'
import { insertTodoBefore } from '../utils/sentinels.ts'

const TANSTACK_ROUTER = '@tanstack/react-router'
const ROOT_ROUTE_DOC = 'https://tanstack.com/router/latest/docs/framework/react/api/router/createRootRouteFunction'
  ...
  for (const child of ret.children()) {
    ...
    if (child.kind() === 'parenthesized_expression') {
      for (const ic of child.children()) {
        if (ic.kind() === 'jsx_element' || ic.kind() === 'jsx_fragment' || ic.kind() === 'jsx_self_closing_element') {
          return ic
        }
      }
    }
...
function rebuildRootJsx(_source: string, htmlEl: SgNode<TSX>): string | null {
  ...
  if (openEl.field('name')?.text() !== 'html') {
    return null
  }
...
function assembleHtmlInner(args: {
  ...
  for (const child of directChildren) {
    ...
    if (child.kind() === 'jsx_text' && child.text().trim() === '') {
      continue
    }
...
function collectImports(rootNode: SgNode<TSX>, globalsUrl: string): Edit[] {
  ...
  const routerImport = addImport(rootNode, {
    type: 'named',
    specifiers: [{ name: 'Outlet' }, { name: 'createRootRoute' }, { name: 'HeadContent' }, { name: 'Scripts' }],
    from: TANSTACK_ROUTER,
  })
  if (routerImport) {
    edits.push(routerImport)
  }

  const cssImport = addDefaultImport(rootNode, globalsUrl, 'appCss')
  if (cssImport) {
    edits.push(cssImport)
  }
...
function rebuildPagesAppJsx(source: string, jsxRoot: SgNode<TSX>): string | null {
  ...
  const next = frag
    .replaceAll(/<Component\s*\{\s*\.\.\.\s*pageProps\s*\}\s*\/>/g, '<Outlet />')
    .replaceAll(/<Component\s*\{\s*\.\.\.\s*pageProps\s*\}\s*>\s*<\/Component>/g, '<Outlet />')
    .replaceAll(/<Component\s*\/>/g, '<Outlet />')
  if (next === frag) {
    return null
  }
  return next
...
function stripAppWithTranslationImport(source: string): string {
  return source.replace(/^\s*import\s+\{\s*appWithTranslation\s*\}\s+from\s+["']next-i18next["']\s*;?\s*\r?\n?/m, '')
}
