...
const codemod: Codemod<TSX> = async (root) => {
  ...
  for (const call of preScanCalls) {
    ...
    if (builtin !== 'revalidateTag' && builtin !== 'revalidatePath') {
      continue
    }
    if (firstArgumentListExpr(call)) {
      invalidateCount++
    }
  }
  ...
  for (const stmt of nextCacheImports) {
    const plan = planNextCacheImportRewrite(stmt, invalidateCount > 0 && qcSpecifier !== null, qcSpecifier)
    if (plan.kind === 'noop') {
      continue
    }
  ...
  for (const call of callSites) {
    ...
    if (builtin === 'unstable_noStore') {
      ...
      if (estmt) {
        edits.push(removeStatementSpan(source, estmt, nextTodoLead()))
      } else {
        edits.push(call.replace('undefined'))
      }
      continue
    }
    ...
    if (builtin === 'revalidatePath') {
      ...
      const keyInner =
        typ !== null && typ !== undefined
          ? `'next-cache', 'path', ${path.text()}, ${typ.text()}`
          : `'next-cache', 'path', ${path.text()}`
      edits.push(call.replace(`${nextTodoLead()}queryClient.invalidateQueries({ queryKey: [${keyInner}] })`))
      continue
    }
...
function queryOptionsInsertAfterExport(
  ...
  if (new RegExp(`\\b${escapeReg(optName)}\\b`).test(source)) {
    return null
  }
  ...
  const staleLine = st !== null && st !== undefined ? `  staleTime: ${st},\n` : ''
...
function unstableCacheTrailingHint(args: SgNode<TSX>[]): string {
  ...
  const parts: string[] = []
  if (keyText) {
    parts.push(`unstable_cache keys ${keyText}`)
  }
  const rev = /revalidate\s*:\s*(\d+)/.exec(optsText)
  if (rev) {
    const sec = Number(rev[1])
    if (!Number.isNaN(sec)) {
      parts.push(`revalidate ${sec}s → staleTime: ${sec * 1000}`)
    }
  }
  const tags = /tags\s*:\s*\[([^\]]*)\]/.exec(optsText)
  if (tags?.[1]) {
    parts.push(`tags [${tags[1].trim()}]`)
  }
  if (parts.length === 0) {
    return ''
  }
...
function queryClientSpecifierForFile(pkgRoot: string, fileAbs: string): string | null {
  ...
  const base = qcAbsTs.replaceAll('\\', '/').replace(/\.ts$/, '')
  let rel = relative(dirname(fileAbs), base).replaceAll('\\', '/')
  if (!rel.startsWith('.')) {
    rel = `./${rel}`
  }
...
function parseImportPiece(raw: string): ImportSpecPiece | null {
  ...
  if (asMatch) {
    return { exported: asMatch[1] ?? '', local: asMatch[2] ?? '', raw: t }
  }
...
function planNextCacheImportRewrite(
  ...
  for (const raw of splitImportSpecifiers(brace)) {
    ...
    if (builtinForExported(p.exported)) {
      continue
    }
    kept.push(p.raw)
  }
  ...
  if (kept.length > 0) {
    lines.push(`import { ${kept.join(', ')} } from "${NEXT_CACHE}";`)
  }
