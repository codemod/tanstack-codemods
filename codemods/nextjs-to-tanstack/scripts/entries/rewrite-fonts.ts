...
const codemod: Codemod<TSX> = async (root) => {
  ...
  const fontSpecifiers: { source: 'google' | 'local'; specifier: string }[] = []
  if (anyGoogle) {
    for (const spec of anyGoogle.findAll({ rule: { kind: 'import_specifier' } })) {
      const name = spec.field('name')?.text()
      if (name) {
        fontSpecifiers.push({ source: 'google', specifier: name })
      }
    }
  }
  if (anyLocal) {
    ...
    if (defaultName) {
      fontSpecifiers.push({ source: 'local', specifier: defaultName })
    }
  }
  if (!anyGoogle && !anyLocal) {
    return null
  }
  void googleImport
  void localImport
  ...
  const fontBindings: { binding: string; entry: FontEntry }[] = []
  ...
  for (const attr of rootNode.findAll({ rule: { kind: 'jsx_attribute' } })) {
    ...
    if (!bindingNames.some((b) => attrContainsFontMember(attr, b))) {
      continue
    }
...
function buildLocalFontFaces(
  ...
  if (srcVal.is('string')) {
    ...
    if (frag) {
      pushResolved(frag.text(), topWeight, topStyle)
    }
    return out
  }
  if (srcVal.is('array')) {
...
function readVariableArg(call: SgNode<TSX>): string | null {
  ...
  for (const arg of args.children()) {
    ...
    for (const pair of arg.findAll({ rule: { kind: 'pair' } })) {
      const parent = pair.parent()
      if (!parent || parent.id() !== arg.id()) {
        continue
      }
      const key = pair.field('key')
      if (key?.text() !== 'variable') {
        continue
      }
...
function tryRewriteMixedClassName(
  ...
  for (const b of bindings) {
    const re = new RegExp(`\\$\\{\\s*${escapeRegex(b)}\\s*\\.\\s*(?:variable|className|style)\\s*\\}`, 'g')
    s = s.replace(re, '')
  }
