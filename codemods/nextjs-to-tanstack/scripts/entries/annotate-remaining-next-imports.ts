...
const codemod: Codemod<TSX> = async (root) => {
  ...
  if (isNextMiddlewarePath(file) && nextImports.length > 0) {
    ...
    if (edits.length > 0) {
      const froms = nextImports.map((s) => parseImportSource(s.text())).filter((x): x is string => Boolean(x))
  ...
  for (const stmt of nextImports) {
    ...
    if (hasTodoSentinel(source, stmt, from)) {
      continue
    }
...
function todoForNextSpecifier(from: string): { message: string; docUrl?: string } {
  ...
  if (from === 'next/link' || from === 'next/image' || from === 'next/router' || from === 'next/compat/router') {
    return {
