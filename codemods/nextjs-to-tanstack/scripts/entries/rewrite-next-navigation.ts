...
const codemod: Codemod<TSX> = async (root) => {
  ...
  if (analyze.fileRouteQueryBindings.size > 0) {
    fileRouteQueryEdits.push(...insertRouteUseParamsHooks(rootNode, source, analyze.fileRouteQueryBindings))
    fileRouteQueryEdits.push(...replaceRouterQueryWithParams(rootNode, analyze.fileRouteQueryBindings))
  }
  ...
  for (const stmt of importStmts) {
    ...
    if (!navMod && !routerMod && !compatMod) {
      continue
    }

    const fromModule = routerMod !== null || compatMod !== null ? NEXT_PAGES_ROUTER : NEXT_NAV
    const importSourceLiteral = navMod !== null ? NEXT_NAV : compatMod !== null ? NEXT_COMPAT_ROUTER : NEXT_PAGES_ROUTER
    ...
    for (const raw of specs) {
      ...
      if (fromModule === NEXT_NAV && rp?.exported === 'useRouter' && /^\s*import\s+type\b/.test(text)) {
    ...
    const onlyTypeUseRouterImport =
      navMod !== null && /^\s*import\s+type\b/.test(text) && inserted === '' && specsAreOnlyTypeUseRouter(specText)
    ...
    if (inserted.replace(/\s+$/, '') === text.replace(/\s+$/, '') && !onlyReadonlyNavigationImport) {
      continue
    }
  ...
  if (!skipUseRouterCallMigrate) {
    ...
    for (const decl of collectDualHookDeclarators(rootNode, analyze.dualHookBindings)) {
      ...
      if (!lexical || countVariableDeclarators(lexical) !== 1) {
        continue
      }
      ...
      if (!/^\s*useRouter\s*\(\s*\)\s*$/.test(initTxt)) {
        continue
      }
    ...
    const navCalleeForBinding = (bindingName: string): string =>
      analyze.dualHookBindings.has(bindingName) ? imperativeNavIdentifier(bindingName) : bindingName
    ...
    for (const call of inlineUseRouterNavCalls(rootNode)) {
      ...
      edits.push(call.replace(buildImperativeNavigationCall('useNavigate()', arg, prop === 'replace')))
      routerNavEdits++
    }
    ...
    for (const name of collectUseRouterBindingNames(rootNode)) {
      ...
      for (const mem of rootNode.findAll({
        ...
        if (parent.field('function')?.id() !== mem.id()) {
          continue
        }
  ...
  if (!skipUseRouterCallMigrate) {
    for (const call of rootNode.findAll({
      ...
      if (!shouldReplaceBareUseRouterCall(call, analyze)) {
        continue
      }
      const r = call.range()
      if (dualLexicalRanges.some((lex) => overlapsRange({ start: r.start.index, end: r.end.index }, lex))) {
        continue
      }
...
function insertAfterImportsBlock(rootNode: SgNode<TSX>, source: string, inserted: string): Edit | null {
  if (source.includes('ReadonlyURLSearchParams = URLSearchParams')) {
    return null
  }
...
function parseImportSpecifier(raw: string): { exported: string; local: string } | null {
  ...
  if (am) {
    return { exported: am[1] ?? '', local: am[2] ?? '' }
  }
...
function collectUseRouterBindingNames(root: SgNode<TSX>): Set<string> {
  ...
  for (const decl of root.findAll({ rule: { kind: 'variable_declarator' } })) {
    ...
    if (callee?.kind() !== 'identifier' || callee.text() !== 'useRouter') {
      continue
    }
...
function canMigrateFileRouteQuery(source: string, bindingName: string, members: Set<string>): boolean {
...
function insertRouteUseParamsHooks(root: SgNode<TSX>, source: string, names: Set<string>): Edit[] {
  ...
  for (const decl of root.findAll({ rule: { kind: 'variable_declarator' } })) {
    ...
    if (braceIdx === -1 || braceIdx >= r.end.index) {
      continue
    }
...
function replaceRouterQueryWithParams(root: SgNode<TSX>, names: Set<string>): Edit[] {
  ...
  for (const me of root.findAll({ rule: { kind: 'member_expression' } })) {
    ...
    if (obj.field('property')?.text() !== 'query') {
      continue
    }
  ...
  for (const name of bindings) {
    ...
    if (canMigrateFileRouteQuery(source, name, members)) {
      ...
      for (const m of members) {
        if (ROUTER_HARD_SKIP_MEMBERS.has(m)) {
          hardSkipped = true
        }
      }
    }
    for (const m of members) {
      if (ROUTER_DUAL_HOOK_MEMBERS.has(m)) {
        dualHookBindings.add(name)
      }
    }
  }
...
function hasInlineNextRouterRisk(root: SgNode<TSX>): boolean {
  for (const mem of root.findAll({ rule: { kind: 'member_expression' } })) {
    ...
    if (c?.kind() !== 'identifier' || c.text() !== 'useRouter') {
      continue
    }
    const p = mem.field('property')?.text()
    if (!p || p === 'push' || p === 'replace') {
      continue
    }
    return true
  }
...
function dualHookLexicalRanges(source: string, root: SgNode<TSX>, dualNames: Set<string>): IndexSpan[] {
  ...
  for (const decl of collectDualHookDeclarators(root, dualNames)) {
    ...
    if (!lex || countVariableDeclarators(lex) !== 1) {
      continue
    }
    const k = lex.id()
    if (seenLex.has(k)) {
...
function historyLikeRouterCallRewrite(prop: string, routerBinding: string): string {
  if (prop === 'refresh') {
    return `${routerBinding}.invalidate()`
  }
  if (prop === 'back') {
    return `${routerBinding}.history.back()`
  }
  if (prop === 'forward') {
    return `${routerBinding}.history.forward()`
  }
...
function buildRedirectThrowEdit(call: SgNode<TSX>, newRedirectCall: string, source: string): Edit | null {
  const p = call.parent()
...
function mergeTanstackImports(specs: string[]): string[] {
  ...
  for (const s of specs) {
    const key = s.replaceAll(/\s+/g, ' ').trim()
    if (!key || seen.has(key)) {
      continue
    }
    seen.add(key)
...
function coalesceAdjacentTanstackImports(plan: { stmt: SgNode<TSX>; text: string }[], source: string): void {
  if (plan.length < 2) {
    return
  }
  const ordered = plan
...
function coalesceFollowingUnchangedTanstackImport(
  ...
  for (let i = 0; i < imports.length - 1; i++) {
    ...
    if (aStmt === undefined || bStmt === undefined) {
      continue
    }
...
function parseNamedImportSpecsFromTanstackLine(text: string): string[] | null {
  const m = /^\s*import\s+\{\s*([^}]+)\s*\}\s*from\s*["']@tanstack\/react-router["']\s*;?\s*$/m.exec(text.trim())
  if (!m?.[1]) {
    return null
  }
    ...
    if (p.kind === 'sub') {
      ...
      if (phase === 'path') {
        ...
        if (hAt >= 0 && (qAt < 0 || hAt < qAt)) {
          if (hAt > 0) {
            pathToks.push({ t: 's', v: text.slice(0, hAt) })
          }
          phase = 'hash'
        ...
        if (qAt >= 0) {
          if (qAt > 0) {
            pathToks.push({ t: 's', v: text.slice(0, qAt) })
          }
          phase = 'query'
...
function isRedirectReplaceArg(arg: SgNode<TSX> | null, redirectTypeLocals: Set<string>): boolean {
  if (!arg || redirectTypeLocals.size === 0) {
    return false
  }
  ...
  if (obj?.kind() !== 'identifier' || prop?.text() !== 'replace') {
    return false
  }
...
function shouldReplaceBareUseRouterCall(call: SgNode<TSX>, analyze: { dualHookBindings: Set<string> }): boolean {
  const p = call.parent()
