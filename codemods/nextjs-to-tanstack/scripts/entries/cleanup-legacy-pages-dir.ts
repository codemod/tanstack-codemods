...
import { mkdirSync, readdirSync, readFileSync, statSync, writeFileSync, type Stats } from 'node:fs'
...
const codemod: Codemod<JSON_TYPES> = async (root) => {
  ...
  const srcPages = handleLegacyPages(join(pkgRoot, 'src/pages'), join(pkgRoot, BACKUP_SUBDIR, 'src-pages'))
