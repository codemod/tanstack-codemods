...
function rewriteNextLinkTypeOnlyImports(rootNode: SgNode<TSX>): Edit[] {
  ...
  for (const stmt of rootNode.findAll({ rule: { kind: 'import_statement' } })) {
    const t = stmt.text()
    if (!/from\s*["']next\/link["']/.test(t)) {
      continue
    }
    if (!isNextLinkTypeOnlyImportStatement(t)) {
      continue
    }
  ...
  for (const opening of opens) {
    ...
    for (const attr of openEl.findAll({ rule: { kind: 'jsx_attribute' } })) {
      ...
      if (prop === 'prefetch') {
        ...
        raw = raw.replaceAll(/^\{/g, '').replaceAll(/\}$/g, '').trim()

        if (raw === 'true') {
          edits.push(attr.replace(`preload="intent"`))
        } else if (raw === 'false') {
          edits.push(attr.replace('preload={false}'))
        } else {
          edits.push(attr.replace(`preload={${raw} ? "intent" : false}`))
        }
        continue
      }
...
function shouldSkipNextLinkMigration(source: string): boolean {
  const importsUseLinkStatus =
    /import\s[^;]*\buseLinkStatus\b[^;]*from\s*["']next\/link["']/m.test(source) || /\buseLinkStatus\s*\(/.test(source)
  const mdxHeavy = /\buseMDXComponents\s*\(/.test(source) || /from\s*["']mdx\/types["']/.test(source)
...
function literalRequiresNativeAnchor(literal: string): boolean {
  ...
  if (/^mailto:/i.test(literal)) {
    return true
  }
  if (/^tel:/i.test(literal)) {
    return true
  }
  if (/^javascript:/i.test(literal)) {
    return true
  }
  return false
}
...
function hrefAttrRequiresNativeAnchor(hrefAttr: SgNode<TSX>): boolean {
  ...
  if (val.kind() === 'jsx_expression') {
    ...
    return inner !== null && inner !== undefined && literalRequiresNativeAnchor(inner)
  }
  return false
}
...
function stripNextLinkOnlyAttrsFromConvertedAnchor(fragment: string): string {
  ...
  for (let i = 0; i < 12; i++) {
    const t = s.replaceAll(new RegExp(`\\s(?:${attrNames})(?:=\\{[^}]*\\}|="[^"]*"|='[^']*')`, 'g'), '')
    if (t === s) {
      break
    }
    s = t
...
function rewriteImage(rootNode: SgNode<TSX>, alias: string): Edit[] {
  ...
  for (const opening of findJsxOpens(rootNode, alias)) {
    ...
    for (const attr of openEl.findAll({ rule: { kind: 'jsx_attribute' } })) {
      ...
      if (['quality', 'unoptimized', 'loader', 'loaderFile', 'objectFit', 'preload'].includes(attrName)) {
        edits.push(dropJsxAttr(attr, source))
      }
    }
