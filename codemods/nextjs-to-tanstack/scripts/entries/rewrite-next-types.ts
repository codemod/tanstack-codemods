...
const codemod: Codemod<TSX> = async (root) => {
  ...
  const imports = rootNode
    ...
    .filter((s) => isNextTypesErasureImport(s));
  ...
  if (typeNames.size === 0 && namespaceNames.size === 0) return null;

  const importRanges = imports.map((i) => i.range());
  const insideImports = (n: SgNode<TSX>): boolean => {
    const r = n.range();
    for (const ir of importRanges) {
      if (r.start.index >= ir.start.index && r.end.index <= ir.end.index) return true;
    }
    return false;
  };
  ...
  for (const name of typeNames) {
    ...
    for (const node of rootNode.findAll({ rule: { kind: "type_identifier", regex: rx } })) {
      if (insideImports(node)) continue;
      if (skipTypeNodeIds.has(node.id())) continue;
      anySites.push(node);
    }
  }
  ...
  for (const nsName of namespaceNames) {
    ...
    for (const nid of rootNode.findAll({ rule: { kind: "nested_type_identifier" } })) {
      if (insideImports(nid)) continue;
      const mod = nid.child(0);
      if (!mod || mod.kind() !== "identifier" || mod.text() !== nsName) continue;
      anySites.push(nid);
    }
  ...
  for (const site of anySites) {
    ...
    if (todoAnchorsSeen.has(a0)) continue;
    ...
    for (const fn of root.findAll({ rule: { kind: fk } })) {
      ...
      for (const ch of fp.children()) {
        if (ch.kind() === "required_parameter" || ch.kind() === "optional_parameter") {
          paramNodes.push(ch as SgNode<TSX>);
        }
      }
      ...
      if (!/^_[a-zA-Z_]/.test(name)) continue;

      const ta = p.find({ rule: { kind: "type_annotation" } });
      const tid = ta?.find({ rule: { kind: "type_identifier" } });
      const typeName = tid?.text();
      if (!typeName || !typeNames.has(typeName)) continue;

      const body = fn.field("body");
      if (!body) continue;
      if (identifierUsedInNodeOutsideRange(body, name, p.range())) continue;

      if (tid) out.add(tid.id());
    }
  }
  return out;
}
...
function statementAnchorForTodo(n: SgNode<TSX>): SgNode<TSX> {
  ...
  for (;;) {
    const par: SgNode<TSX> | null = cur.parent() as SgNode<TSX> | null;
    if (!par) return cur;
    const pk = par.kind();
    if (pk === "program" || pk === "statement_block") return cur;
    cur = par;
  }
}
...
function isNextTypesErasureImport(stmt: SgNode<TSX>): boolean {
  ...
  if (f === MODULE_TYPES) return true;
  if (f === MODULE_NEXT && isTypeOnlyImportStatement(stmt)) return true;
...
function extractNamedAndDefaultLocals(stmt: SgNode<TSX>): string[] {
  ...
  if (clause) {
    for (const c of clause.children()) {
      if (c.kind() === "named_imports" || c.kind() === "namespace_import") break;
...
function identifierUsedInNodeOutsideRange(
  ...
  for (const id of scope.findAll({ rule: { kind: "identifier", regex: rx } })) {
    const r = id.range();
    if (r.start.index >= exclude.start.index && r.end.index <= exclude.end.index) continue;
    return true;
  }
  return false;
}
