...
const codemod: Codemod<TSX> = async (root) => {
  ...
  if (dynamicAlias) {
    ...
    if (dynamicEdits !== 'skip') {
      edits.push(...dynamicEdits)
    }
  }
...
function rewriteDynamic(rootNode: SgNode<TSX>, alias: string): Edit[] | 'skip' {
  ...
  for (const call of calls) {
    ...
    if (!isSimpleImportArrow(loader)) {
      return 'skip'
    }
    if (args.length === 2) {
      const opt = args[1]
      if (!opt || !canConvertDynamicOptionsToLazy(opt)) {
        return 'skip'
      }
      if (objectOptionHasLoadingKey(opt)) {
        strippedLoadingUi = true
      }
    }
    callEdits.push(call.replace(`lazy(${loader.text()})`))
  }

  if (callEdits.length === 0) {
    return []
  }
...
function canConvertDynamicOptionsToLazy(node: SgNode<TSX>): boolean {
  if (isSafeDynamicOptionsNode(node)) {
    return true
  }
  if (!node.is('object')) {
    return false
...
function isSimpleImportArrow(arg: SgNode<TSX>): boolean {
  ...
  if (body.startsWith('(') && /import\s*\(/.test(body)) {
    return true
  }
  return false
}
...
function rewriteScript(rootNode: SgNode<TSX>, alias: string): Edit[] | 'skip' {
  if (!scriptUsagesAreAllSafe(rootNode, alias)) {
    return 'skip'
  }
  ...
  for (const opening of findJsxScriptOpens(rootNode, alias)) {
    ...
    if (strat) {
      ...
      const v = raw.replace(/^=\s*/, '').replaceAll(/["']/g, '').trim()
      if (/beforeInteractive|^worker$/i.test(v)) {
        return 'skip'
      }
      if (v === 'lazyOnload') {
        useAsync = !!srcAttr
    ...
    if (target.kind() === 'jsx_element') {
      ...
      if (close) {
        const ct = close.text()
        const nextClose = ct.replace(new RegExp(`</\\s*${escapeRegex(alias)}\\s*>`, 'i'), '</script>')
        if (nextClose !== ct) {
          edits.push(close.replace(nextClose))
        }
      }
    }
  }
...
function patchScriptOpeningText(openText: string, alias: string, useDefer: boolean, useAsync: boolean): string | null {
  let t = openText.replace(new RegExp(`^<\\s*${escapeRegex(alias)}\\b`), '<script')
  if (!/^<\s*script\b/.test(t)) {
    return null
  }
  t = t.replaceAll(/\s+strategy=\{[^}]*\}\s*/g, ' ')
  t = t.replaceAll(/\s+strategy="[^"]*"\s*/gi, ' ')
  t = t.replaceAll(/\s+strategy='[^']*'\s*/gi, ' ')
  t = t.replaceAll(/\s{2,}/g, ' ')
  if (useAsync && !/\basync\b/.test(t)) {
    t = t.replace(/<script\b/, '<script async')
  } else if (useDefer && !/\bdefer\b/.test(t)) {
    t = t.replace(/<script\b/, '<script defer')
  }
  return t
}
...
function scriptUsagesAreAllSafe(rootNode: SgNode<TSX>, alias: string): boolean {
  for (const opening of findJsxScriptOpens(rootNode, alias)) {
    ...
    if (!hasSrc) {
      if (outer.kind() === 'jsx_self_closing_element') {
        return false
      } // self-closing without src → skip
    }
    if (outer.kind() === 'jsx_element') {
      for (const ch of outer.children()) {
        if (ch.kind() === 'jsx_text' && /\S/.test(ch.text())) {
          return false
        }
      }
    }
...
function outerJsxReplacementTarget(opening: SgNode<TSX>): SgNode<TSX> {
  if (opening.kind() === 'jsx_self_closing_element') {
    return opening
  }
  let cursor: SgNode<TSX> | null = opening
  while (cursor) {
    if (cursor.kind() === 'jsx_element') {
      return cursor
    }
