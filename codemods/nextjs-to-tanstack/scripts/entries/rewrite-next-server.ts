...
const codemod: Codemod<TSX> = async (root) => {
  ...
  if (/(?:NextRequest|NextResponse|NextURL)\s+as\s+/.test(source)) {
    return null
  }
  if (/\*\s*as\s+\w+\s+from\s*["']next\/server["']/.test(source)) {
    return null
  }
  ...
  for (const stmt of importStmts) {
    ...
    if (plan.removedNextPrimitive) {
      stripNextPrimitivesFromImports = true
    }
  ...
  for (const call of rootNode.findAll({ rule: { kind: 'call_expression' } })) {
    ...
    if (obj.text() !== 'NextResponse' && obj.text() !== 'Response') {
      continue
    }
    if (prop !== 'rewrite') {
      continue
