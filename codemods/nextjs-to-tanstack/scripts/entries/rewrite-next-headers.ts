...
const codemod: Codemod<TSX> = async (root) => {
  ...
  if (!cookiesNm && !headersNm) {
    return null
  }

  if (cookiesNm && unsupportedCookiesFactories(rootNode, cookiesNm)) {
    return null
  }
  if (headersNm && unsupportedHeadersFactories(rootNode, headersNm)) {
    return null
  }
  ...
  if (!needs.getCookie && !needs.getHeaders && !needs.setCookie && !needs.deleteCookie && !needs.getCookies) {
    return null
  }
  ...
  for (const stmt of hdrImports) {
    ...
    if (plan.kind === 'delete') {
      edits.push(blankStmt(source, stmt))
    } else if (plan.text + nl !== stmt.text()) {
      edits.push(stmt.replace(`${take()}${plan.text}${nl}`))
    }
  }
  ...
  for (const outer of rootNode.findAll({ rule: { kind: 'call_expression' } })) {
    ...
    if (cookiesRoot) {
      if (!COOKIE_STORE_METHODS.has(prop)) {
        continue
      }
      ...
      if (prop === 'getAll') {
        edits.push(outer.replace(`${take()}Object.entries(getCookies()).map(([name, value]) => ({ name, value }))`))
        continue
      }
    }
  ...
  for (const varName of storedCookieVars) {
    for (const outer of rootNode.findAll({ rule: { kind: 'call_expression' } })) {
      ...
      const prop = fn.field('property')?.text()
      if (!prop || !COOKIE_STORE_METHODS.has(prop)) {
        continue
      }
      const obj = fn.field('object')
      if (obj?.kind() !== 'identifier' || obj.text() !== varName) {
        continue
      }
    ...
    for (const decl of rootNode.findAll({ rule: { kind: 'variable_declarator' } })) {
      ...
      if (nameNode?.kind() !== 'identifier' || nameNode.text() !== varName) {
        continue
      }
      ...
      if (callee?.kind() !== 'identifier' || callee.text() !== cookiesNm) {
        continue
      }
      if (!isZeroArgCall(base)) {
        continue
      }
      // Walk up to find the enclosing lexical_declaration (const/let/var).
      const lexicalDecl = findParentLexicalDeclaration(decl)
      if (lexicalDecl) {
        edits.push(blankStmtFullLine(source, lexicalDecl))
      }
    }
  }
  ...
  if (cookiesNm) {
    for (const c of rootNode.findAll({
      ...
      if (isSupportedCookieFactoryRoot(c)) {
        continue
      }
      if (!isBareHeadersCookiesFactory(c)) {
        continue
      }
      // Skip vars that are handled by the stored-var path above.
      const storedVarName = getStoredVarNameForFactory(c)
      if (storedVarName && storedCookieVars.has(storedVarName)) {
        continue
      }
  ...
  if (headersNm) {
    for (const h of rootNode.findAll({
      ...
      if (isSupportedHeadersFactoryRoot(h)) {
        continue
      }
      if (!isBareHeadersCookiesFactory(h)) {
        continue
      }
  ...
  if (cookieBindingsNeedValueStrip.size > 0) {
    for (const mem of rootNode.findAll({ rule: { kind: 'member_expression' } })) {
      if (mem.field('property')?.text() !== 'value') {
        continue
      }
      ...
      if (!cookieBindingsNeedValueStrip.has(o.text())) {
        continue
      }
...
function scanNeeds(
  ...
  storedCookieVars = new Set<string>(),
): Needs {
  const needs: Needs = {
    getCookie: false,
    getHeaders: false,
  ...
  for (const outer of rootNode.findAll({ rule: { kind: 'call_expression' } })) {
    ...
    if (cookiesNm && unwrapToCookiesFactory(obj, cookiesNm)) {
      if (prop === 'get' && firstArg(outer.field('arguments'))) {
        needs.getCookie = true
      }
      if (prop === 'set') {
        needs.setCookie = true
      }
      if (prop === 'delete') {
        needs.deleteCookie = true
      }
      if (prop === 'has') {
        needs.getCookie = true
      }
      if (prop === 'getAll') {
        needs.getCookies = true
      }
    }
    ...
    if (headersNm && unwrapToHeadersFactory(obj, headersNm) && prop === 'get') {
      if (firstArg(outer.field('arguments'))) {
        needs.getHeaders = true
      }
    }
  }
...
function bareReplacementTarget(factoryCall: SgNode<TSX>): SgNode<TSX> {
  ...
  if (p?.kind() === 'await_expression') {
    ...
    if (inner && inner.id() === factoryCall.id()) {
      return p
    }
  }
  return factoryCall
...
function cookieGetBindingNames(root: SgNode<TSX>, cookiesNm: string): Set<string> {
  ...
  for (const decl of root.findAll({ rule: { kind: 'variable_declarator' } })) {
    ...
    if (!isCookieGetCallExpr(init, cookiesNm)) {
      continue
    }
...
function isCookieGetCallExpr(n: SgNode<TSX>, cookiesNm: string): boolean {
  ...
  if (factoryBase?.kind() !== 'call_expression') {
    return false
  }
...
function buildImportRewrite(stmtText: string, locals: Map<string, Builtin>, needs: Needs): ImportRewrite | null {
  ...
  const names: string[] = []
  if (needs.deleteCookie) {
    names.push('deleteCookie')
  }
  if (needs.getCookie) {
    names.push('getCookie')
  }
  if (needs.getCookies) {
    names.push('getCookies')
  }
  if (needs.getHeaders) {
    names.push('getHeaders')
  }
  if (needs.setCookie) {
    names.push('setCookie')
  }
  ...
  if (kept.length) {
    lines.push(`import { ${kept.join(', ')} } from "${NEXT_HEADERS}";`)
  }
...
function parsePiece(raw: string): { exported: string; local: string; raw: string } | null {
  ...
  if (am) {
    return { exported: am[1] ?? '', local: am[2] ?? '', raw: t }
  }
...
function extractBindings(stmts: SgNode<TSX>[]): Map<string, Builtin> {
  ...
  for (const stmt of stmts) {
    ...
    for (const r of splitSpecs(brace)) {
      ...
      if (p.exported === 'cookies') {
        out.set(p.local, 'cookies')
      } else if (p.exported === 'headers') {
        out.set(p.local, 'headers')
      } else if (p.exported === 'draftMode') {
        out.set(p.local, 'draftMode')
      }
    }
  }
  return out
