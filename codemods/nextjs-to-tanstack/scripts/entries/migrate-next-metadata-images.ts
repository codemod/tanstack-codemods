...
const codemod: Codemod<TSX> = async (root) => {
  ...
  const getSig = paramNames.length === 0 ? 'async ()' : 'async ({ params }: { params: Record<string, string> })'

  const paramPrologue = paramNames.length === 0 ? '' : `        const { ${paramNames.join(', ')} } = params;\n`

  const { importLines, restBlock } = buildPreservedParts(rootNode, defaultExport)
  const needsRouterImport =
    !importLines.join('\n').includes('createFileRoute') && !importLines.join('\n').includes(TANSTACK_ROUTER)
  const routerImport = needsRouterImport ? `import { createFileRoute } from "${TANSTACK_ROUTER}";` : ''
  ...
  let out = rootNode.commitEdits([{ startPos: start.index, endPos: end.index, insertedText: newFile }])
...
function stripAwaitParamsDestructures(body: string): string {
  return body
    .replaceAll(/^\s*const\s+\{[^}]*\}\s*=\s*await\s+params\s*;\s*\r?\n/gm, '')
    .replaceAll(/^\s*const\s+\w+\s*=\s*await\s+params\s*;\s*\r?\n/gm, '')
}
...
function buildPreservedParts(
  ...
  for (const stmt of rootNode.children()) {
    if (stmt.range().start.index === skipStart) {
      continue
    }
    ...
    if (/^export\s+const\s+alt\s*=/.test(t)) {
      continue
    }
    if (/^export\s+const\s+contentType\s*=/.test(t)) {
      continue
    }
    if (/^export\s+type\s+Props\b/.test(t)) {
      continue
    }
    if (/^type\s+Props\b/.test(t)) {
      continue
    }
    if (/^export\s+interface\s+Props\b/.test(t)) {
      continue
    }
    if (/^interface\s+Props\b/.test(t)) {
      continue
    }
...
function coerceAppRouterRelative(appRel: string, absFile: string): string {
  ...
  if (n.startsWith('src/app/') || n.startsWith('app/')) {
    return n
  }
