...
const codemod: Codemod<CSS> = async (root) => {
  ...
  if (googleFonts.length === 0 && localFonts.length === 0) {
    return null
  }
...
function buildLocalFontFaceBlock(source: string, globalsAbs: string, fonts: FontEntry[]): string {
  ...
  for (const font of fonts) {
    ...
    for (const face of faces) {
      ...
      if (source.includes(markerComment)) {
        continue
      }

      const absFont = join(pkgRoot, ...face.repoRelativePath.split(/[/\\]/).filter(Boolean))
      let rel = normalizePath(relative(globalsDir, absFont)).replaceAll('\\', '/')
...
function cssFontFamilyName(font: FontEntry): string {
  if (font.fontFaceFamily) {
    return font.fontFaceFamily
  }
...
function bindingToDisplayFamily(binding: string): string {
  const spaced = binding.replaceAll(/([a-z0-9])([A-Z])/g, '$1 $2')
  return spaced
    .split(/\s+/)
    .filter(Boolean)
...
function collectThemeLines(source: string, googleFonts: FontEntry[], localFonts: FontEntry[]): string[] {
  const themeLines: string[] = []

  for (const font of googleFonts) {
    const varName = font.variable ?? inferVarName(font)
    if (themeVarDeclared(source, varName)) {
      continue
    }
  ...
  for (const font of localFonts) {
    ...
    themeLines.push(`  ${varName}: '${escapeCssFamilyName(displayFamily)}', ui-sans-serif, sans-serif;`)
  }
...
function findThemeInlineBlockRange(source: string): { bodyStart: number; closeBrace: number } | null {
...
function tryMergeThemeLines(source: string, themeLines: string[]): { pos: number; text: string } | null {
