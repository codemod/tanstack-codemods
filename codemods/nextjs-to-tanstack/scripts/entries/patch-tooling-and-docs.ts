...
function patchTsconfig(path: string): void {
  ...
  if (Array.isArray(coerce.plugins)) {
    ...
    if ((coerce.plugins as unknown[]).length === 0) {
      delete coerce.plugins
    }
  }
  ...
  if (types === undefined) {
    ...
    if (!list.includes('vite/client')) {
      coerce.types = [...list, 'vite/client']
    }
  }
...
function patchEslintrcJson(path: string): void {
  ...
  const before = JSON.stringify(cfg)
  const drop = new Set(['next/core-web-vitals', 'next', 'eslint-config-next', 'plugin:@next/next/recommended'])
...
function patchNextReferencesInText(text: string): string {
  let t = text
  t = t.replaceAll(/\bnext\s+build\b/g, 'vite build')
  t = t.replaceAll(/\bnext\s+dev\b/g, 'vite dev')
  t = t.replaceAll(/\bnext\s+start\b/g, 'node .output/server/index.mjs')
  t = t.replaceAll(/\bnext\s+lint\b/g, 'eslint .')
  t = t.replaceAll(/\bnext\.config\.(js|cjs|mjs|ts|mts|cts)\b/g, 'vite.config.ts')
  return t
}
