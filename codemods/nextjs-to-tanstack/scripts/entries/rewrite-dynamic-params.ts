...
const codemod: Codemod<TSX> = async (root) => {
  ...
  if (paramKind.has('params')) {
    awaitKind.set('params', 'params')
  }
  if (paramKind.has('searchParams')) {
    awaitKind.set('searchParams', 'searchParams')
  }
...
function findPageComponentFunction(rootNode: SgNode<TSX>): SgNode<TSX> | null {
  ...
  for (const child of rootNode.children()) {
    if (child.kind() === 'function_declaration') {
      if (child.field('name')?.text() === componentName) {
        return child
      }
    }
  }
...
function rewriteAwaitStatements(
    ...
    if (isCatchAll && kind === 'params') {
      ...
      if (pattern) {
        ...
        if (keys.length === 1) {
          ...
          edits.push(
            insertReviewBefore(decl, 'catch-all route destructure had multiple keys — rewrite to { _splat } by hand'),
          )
        }
      }
    }
  ...
  for (const aw of bareAwaits) {
    if (handledDeclaratorAwaitIds.has(aw.id())) {
      continue
    }
...
function rewriteCatchAllJoinedParamRefs(pageFn: SgNode<TSX>, oldName: string): Edit[] {
  ...
  for (const call of pageFn.findAll({ rule: { kind: 'call_expression' } })) {
    ...
    if (obj?.kind() !== 'identifier' || obj.text() !== oldName) {
      continue
    }
    if (prop?.text() !== 'join') {
      continue
 ...
 */
function needsCatchAllArrayAliasAfterJoinRewrite(pageFn: SgNode<TSX>, oldKey: string, decl: SgNode<TSX>): boolean {
  for (const id of pageFn.findAll({
    rule: { kind: 'identifier', regex: `^${escapeRxIdent(oldKey)}$` },
  })) {
    if (isIdentifierInCatchAllDestructuringPattern(decl, id)) {
      continue
    }
    if (isTypeOnlyIdentifierContext(id)) {
      continue
    }
    if (isOldKeyOnlyUsedAsJoinReceiver(id)) {
      continue
    }
    return true
  }
...
function isOldKeyOnlyUsedAsJoinReceiver(id: SgNode<TSX>): boolean {
  ...
  if (p.field('object')?.id() !== id.id()) {
    return false
  }
  if (p.field('property')?.text() !== 'join') {
    return false
  }
  const call = p.parent()
...
function insertCatchAllArrayAliasAfterDeclarator(decl: SgNode<TSX>, oldKey: string, source: string): Edit[] {
