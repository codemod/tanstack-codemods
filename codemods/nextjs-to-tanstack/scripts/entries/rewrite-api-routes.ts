...
import { computeRoutePath, detectNextFileKind, stripAppPrefix, stripPagesPrefix } from '../utils/route-path.ts'
...
function migrateDefaultExportPagesApiViaFs(
  ...
  if (!source.includes('NextApiRequest') || !source.includes('NextApiResponse')) {
    return null
  }
...
function isAppApiLeafModule(relative: string): boolean {
  ...
  const { rest } = split
  if (rest.length < 2 || rest[0] !== 'api') {
    return false
  }
  const leaf = rest.at(-1) ?? ''
  if (/^route\.(t|j)sx?$/.test(leaf)) {
    return false
  }
...
function normalizeDefaultExportFn(exportStmt: SgNode<TSX>): SgNode<TSX> | null {
  ...
  if (v.kind() === 'function_declaration') {
    return v
  }
...
function findExportWithDefaultFn(rootNode: SgNode<TSX>): SgNode<TSX> | null {
  for (const stmt of rootNode.children()) {
    ...
    if (!stmt.text().includes('default')) {
      continue
    }
    if (normalizeDefaultExportFn(stmt)) {
      return stmt
    }
  }
  return null
...
function looksLikeNextPagesApiHandler(fn: SgNode<TSX>, fileSource: string): boolean {
  ...
  if (paramText.includes('NextApiRequest') && paramText.includes('NextApiResponse')) {
    return true
  }
  return fileSource.includes('NextApiRequest') && fileSource.includes('NextApiResponse')
}
...
function tryMigrateDefaultExportPagesApi(
  ...
  if (!looksLikeNextPagesApiHandler(fn, source)) {
    return null
  }
  ...
  const bodyInner = bodyText.startsWith('{') && bodyText.endsWith('}') ? bodyText.slice(1, -1).trim() : bodyText
  ...
  const out = rootNode.commitEdits([{ startPos: start.index, endPos: end.index, insertedText: newFile }])
...
function handlerFromConstMethodExport(exportStmt: SgNode<TSX>): Handler | null {
  const lex =
    firstChildOfKind(exportStmt, 'lexical_declaration') ?? firstChildOfKind(exportStmt, 'variable_declaration')
  if (!lex) {
    return null
  }
...
function normalizeCallableHttpHandlerValue(value: SgNode<TSX>): SgNode<TSX> | null {
  ...
  if (value.kind() === 'parenthesized_expression') {
    const inner =
      value.children().find((c) => c.kind() === 'arrow_function' || c.kind() === 'function_expression') ?? null
    return inner
  }
  return null
}
...
function buildRouteBlock(routePath: string, handlers: Handler[], inlineImport: boolean): string {
  const importLine = inlineImport ? `import { createFileRoute } from "@tanstack/react-router";\n\n` : ''
