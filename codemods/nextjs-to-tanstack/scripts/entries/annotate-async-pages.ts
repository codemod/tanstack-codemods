...
const codemod: Codemod<TSX> = async (root) => {
  ...
  for (const configObj of collectCreateFileRouteConfigObjects(rootNode)) {
    if (!getObjectPairValue(configObj, "loader")) continue;
    const repair = stripStandaloneCommaLineBeforeLoaderEdit(source, configObj);
    if (repair) edits.push(repair);
  }
...
type RouteCandidate = { fn: SgNode<TSX>; configObj: SgNode<TSX> };
...
function tryBuildLoaderMigrationEdits(
  ...
  if (!split || split.loaderStmts.length === 0) return null;
  const { loaderStmts, ret } = split;

  if (!declaratorsAreSimpleBindings(loaderStmts)) return null;

  for (const aw of ret.findAll({ rule: { kind: "await_expression" } })) {
    if (awaitDepthWithinAsyncBoundary(aw, fn) === 0) return null;
  }

  for (const aw of body.findAll({ rule: { kind: "await_expression" } })) {
    if (awaitDepthWithinAsyncBoundary(aw, fn) !== 0) continue;
    if (!loaderStmts.some((stmt) => rangeContainsNode(stmt, aw))) return null;
  }
  ...
  const ordered =
    usedInSuffix.size > 0 ? declared.filter((n) => usedInSuffix.has(n)) : [...declared];
  ...
  if (declared.length > 0 && exportKeys.length === 0) return null;
...
const ALLOWED_LOADER_STMT_KINDS = new Set([
  "lexical_declaration",
...
function splitLoaderBlockBeforeReturn(
  ...
  for (const s of loaderStmts) {
    if (!ALLOWED_LOADER_STMT_KINDS.has(s.kind())) return null;
    if (statementContainsJsx(s)) return null;
    if (statementContainsReactHookCall(s)) return null;
  }
...
function collectDeclaredNames(leading: SgNode<TSX>[]): string[] {
  ...
  for (const s of leading) {
    ...
    for (const d of s.findAll({ rule: { kind: "variable_declarator" } })) {
      const name = d.field("name");
      if (name?.is("identifier")) names.push(name.text());
    }
  }
  return names;
}
...
function collectIdentifiersUsedFromOuterBindings(
  ...
  function walk(n: SgNode<TSX>, locals: Set<string>) {
    ...
    if (k === "identifier") {
      const t = n.text();
      if (outerBindings.has(t) && !locals.has(t)) used.add(t);
      return;
    }
...
function firstBindingIdentifierInParameter(param: SgNode<TSX>): string | null {
  for (const c of param.children()) {
    if (c.kind() === "identifier") return c.text();
    if (c.kind() === "assignment_pattern") {
      const left = c.field("left");
      if (left?.is("identifier")) return left.text();
    }
  }
  return null;
}
