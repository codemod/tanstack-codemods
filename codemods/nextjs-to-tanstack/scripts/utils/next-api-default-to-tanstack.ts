...
export function rewriteReqQueryToParams(source: string, hasPathParams: boolean): string {
  ...
  let out = source
  out = out.replace(/const\s*\{\s*([a-zA-Z0-9_$,\s]+)\s*\}\s*=\s*req\.query\s*;/, (_, keys: string) => {
    const parts = keys
      .split(',')
      .map((k) => k.trim())
      .filter(Boolean)
  ...
  out = out.replaceAll(/\breq\.query\.(\w+)/g, 'params.$1')
  return out
}
...
export function rewriteReqQueryFromSearchParams(source: string): string {
  ...
  out = out.replaceAll(/\breq\.query\.(\w+)/g, 'query.$1')
  out = out.replaceAll(/\breq\.query\b/g, 'query')
  return out
}
...
export function rewriteReqBodyFromJson(source: string): string {
  if (!/\breq\.body\b/.test(source)) {
    return source
  }
  let out = source.replaceAll(/\breq\.body\b/g, 'body')
...
export function replaceResStatusEndCalls(source: string, options?: { allow405Method?: 'GET' | 'POST' }): string {
...
export function inferDefaultExportPagesApiKind(bodyInner: string): 'GET' | 'POST' {
  const t = bodyInner
  if (/\breq\.body\b/.test(t)) {
    return 'POST'
  }
  if (/req\.method\s*!==\s*["']POST["']/.test(t)) {
    return 'POST'
  }
  if (/req\.method\s*===\s*["']POST["']/.test(t)) {
    return 'POST'
...
export function isMultiMethodNextHandler(bodySource: string): boolean {
  if (/\bmethod\s*===\s*["']POST["']/.test(bodySource)) {
    return true
  }
  if (/\bmethod\s*===\s*["']PUT["']/.test(bodySource)) {
    return true
  }
  if (/\bmethod\s*===\s*["']DELETE["']/.test(bodySource)) {
    return true
  }
  if (/\bmethod\s*===\s*["']PATCH["']/.test(bodySource)) {
    return true
  }
  if (/switch\s*\(\s*method\s*\)/.test(bodySource)) {
    return true
  }
  return false
}
