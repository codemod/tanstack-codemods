...
function skipTsxElement(source: string, start: number): number {
  ...
  if (!isLikelyTsxJsxOpen(source, start)) {
    return start + 1
  }
  ...
  while (i < source.length) {
    ...
    while (i < source.length) {
      ...
      if (attrCh === undefined || !/[\w:$-]/.test(attrCh)) {
        break
      }
      i++
    }
    ...
    if (i < source.length && source[i] === '=') {
      ...
      if (q === '"' || q === "'") {
        ...
        while (i < source.length) {
          ...
          if (valCh === undefined || /\s/.test(valCh) || valCh === '>') {
            break
          }
          i++
        }
