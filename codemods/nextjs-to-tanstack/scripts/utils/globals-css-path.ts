 ...
 */
function inferGlobalsFromImports(source: string, layoutFileAbs: string, appDirOfRoot: string): string | null {
  ...
  for (const re of [importRe, sideRe]) {
    ...
    while (m !== null) {
      const spec = m[1]
      if (spec === undefined || !spec.includes('globals.css')) {
        continue
      }
...
function resolveCssImportSpecifier(spec: string, layoutDir: string, pkgRoot: string): string | null {
  const s = spec
  if (s.startsWith('@/')) {
