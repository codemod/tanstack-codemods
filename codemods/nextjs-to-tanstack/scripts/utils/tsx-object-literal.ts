...
export function objectLiteralOpenBraceIndex(source: string, obj: SgNode<TSX>): number {
  ...
  for (const child of obj.children()) {
    ...
    minPairStart = Math.min(minPairStart, utf8ByteOffsetToUtf16Index(source, child.range().start.index))
  }
  ...
  if (braceChild) {
    return utf8ByteOffsetToUtf16Index(source, braceChild.range().start.index)
  }
