...
export function getImport<T extends TypesMap>(program: SgNode<T>, options: GetOpts): ReturnType<typeof rawGetImport> {
...
export function removeImport<T extends TypesMap>(program: SgNode<T>, options: RemoveOpts): Edit | null {
  return (rawRemoveImport as unknown as RawFn)(program, options) as Edit | null
}

export function removeNamedImports<T extends TypesMap>(program: SgNode<T>, from: string, specifiers: string[]): Edit[] {
  const edits: Edit[] = []
...
export function addDefaultImport<T extends TypesMap>(program: SgNode<T>, from: string, name: string): Edit | null {
 ...
 */
export function tanstackRouterNamedImportCommaFixEdits<T extends TypesMap>(program: SgNode<T>): Edit[] {
  const edits: Edit[] = []
  const untyped = program as unknown as SgNode<TypesMap>
  for (const stmt of untyped.findAll({ rule: { kind: 'import_statement' } })) {
    const t = stmt.text()
    if (!/from\s*["']@tanstack\/react-router["']/.test(t)) {
      continue
    }
    const fixed = t.replaceAll(/([\w$])\s+,/g, '$1,')
    if (fixed !== t) {
      edits.push({
