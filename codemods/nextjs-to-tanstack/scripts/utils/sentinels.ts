 ...
 * markers.
 */
export function hasReviewSentinel<T extends TypesMap>(source: string, node: SgNode<T>, needle?: string): boolean {
  return hasSentinel(source, node as unknown as AnyNode, REVIEW_PREFIX, needle)
}

export function hasTodoSentinel<T extends TypesMap>(source: string, node: SgNode<T>, needle?: string): boolean {
...
function hasSentinel(source: string, node: AnyNode, prefix: string, needle?: string): boolean {
  ...
  for (const line of precedingLines) {
    ...
    if (needle && !trimmed.includes(needle)) {
      continue
    }
    return true
  }
